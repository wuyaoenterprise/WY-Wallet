...
if tax or service_charge or discount:
    st.info(
        f"AI 识别附加项：税 {money(tax)} · 服务费 {money(service_charge)} · 折扣 {money(discount)}。"
        "这些金额已经作为可见的可编辑交易行加入下方，不会只参与验算却漏记到账本。"
    )
...
st.subheader("检查并修改")
st.caption("看不清日期时会暂填今天但必须人工确认；退款和折扣使用 Refund 抵减支出。税费、服务费和折扣都已进入编辑表，可修改或取消。")
...
# Tax/service/discount have already been materialized as editable transaction
# rows. Reconciliation therefore compares the rows that will actually be saved
# directly with the receipt grand total, preventing an "验算正确但漏记费用" gap.

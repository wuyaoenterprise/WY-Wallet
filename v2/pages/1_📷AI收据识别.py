...
with upload_tab:
    uploaded = st.file_uploader("上传 JPG、PNG 或 WebP", type=["jpg", "jpeg", "png", "webp"], key="receipt_upload_v5")
with camera_tab:
    captured = st.camera_input("拍摄收据", key="receipt_camera_v5")
...
st.subheader("检查并修改")
st.caption("看不清日期时会暂填今天但必须人工确认；退款使用 Refund，不计入收入，而是抵减对应类别支出。疑似重复默认不保存，可明确强制保存。")
...
d.metric("退款", money(refund_total))
e.metric("净支出", money(expense_total - refund_total))

reconciliation = reconcile_receipt_total(
    candidates,
    payload.get("receipt_total"),
    tax=payload.get("tax", 0),
    service_charge=payload.get("service_charge", 0),
    discount=payload.get("discount", 0),
)
difference_needs_confirm = False
if reconciliation:
    st.caption(
        f"项目 {money(reconciliation['item_total'])} + 税 {money(reconciliation['tax'])} + 服务费 {money(reconciliation['service_charge'])} "
        f"− 折扣 {money(reconciliation['discount'])} = {money(reconciliation['expected_total'])}"
    )
    if reconciliation["matches"]:
        st.success(f"本地验算与收据总额 {money(reconciliation['receipt_total'])} 一致（容差 RM {reconciliation['tolerance']:.2f}）。")
    else:
        difference_needs_confirm = True
        st.warning(f"本地验算与收据总额 {money(reconciliation['receipt_total'])} 相差 {money(abs(reconciliation['difference']))}。请检查漏项、税费、服务费、折扣或退款方向。")

confirm_difference = True
if difference_needs_confirm:
    confirm_difference = st.checkbox("我已检查总额差异，仍确认按当前项目保存。", key="receipt_difference_confirm_v2")

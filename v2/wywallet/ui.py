...
<style>
:root {
  --wy-primary:#5b8ff9; --wy-positive:#35b77e; --wy-negative:#ef6464;
  --wy-warning:#f6bd16; --wy-border:rgba(128,128,128,.24);
...
.wy-card,.wy-detail{border:1px solid var(--wy-border);border-radius:14px;padding:1rem;background:rgba(127,127,127,.025)}
.wy-detail{margin-top:.7rem}.wy-chip{display:inline-block;border:1px solid var(--wy-border);border-radius:999px;padding:.12rem .5rem;font-size:.78rem;color:var(--wy-muted)}
...
[data-testid="stPlotlyChart"] {border:1px solid var(--wy-border);border-radius:16px;padding:.35rem .45rem .1rem;background:linear-gradient(180deg,rgba(255,255,255,.035),rgba(255,255,255,.012));overflow:hidden;}
[data-testid="stPlotlyChart"] [data-testid="stElementToolbar"]{display:none!important}
[data-testid="stPlotlyChart"] .js-plotly-plot,[data-testid="stPlotlyChart"] .plot-container{touch-action:pan-y!important}
@media(max-width:760px){
 [data-testid="stAppViewContainer"] > .main .block-container{padding-left:.7rem;padding-right:.7rem}
 .wy-page-title{font-size:1.55rem}.wy-calendar{gap:3px}.wy-calendar-day{min-height:48px;padding:.25rem}.wy-calendar-amount{font-size:.66rem;margin-top:.2rem}
 [data-testid="stPlotlyChart"]{border-radius:12px;padding:.15rem .1rem 0}
}
</style>
"""
...
def safe_detail_html(item: str, category: str, type_label: str, amount: float, tx_date: str, note: str, income: bool) -> str:
    item_e = html.escape(str(item))
    category_e = html.escape(str(category))
    type_e = html.escape(str(type_label))
    date_e = html.escape(str(tx_date))
    note_e = html.escape(str(note or ""))
    amount_class = "wy-amount-income" if income else "wy-amount-expense"
    sign = "+" if income else "−"
    note_suffix = f" · {note_e}" if note_e else ""
    return (
        f'<div class="wy-detail"><span class="wy-chip">{type_e}</span> '
        f'<span class="wy-chip">{category_e}</span><h3 style="margin:.55rem 0 .2rem">{item_e}</h3>'
        f'<div class="{amount_class}" style="font-size:1.35rem">{sign}{money(amount)}</div>'
...
def polish_figure(fig: go.Figure) -> go.Figure:
    ...
    try:
        ...
        fig.update_traces(marker=dict(line=dict(color="rgba(255,255,255,.18)", width=1.5)), textfont=dict(size=12), pull=0, selector=dict(type="pie"))
    except Exception:
        pass
    return fig
...
def render_chart(fig: go.Figure, *, height: int | None = None, legend: bool | None = None, hovermode: str | None = None) -> None:
    if height is not None or legend is not None or hovermode is not None:
        updates = {}

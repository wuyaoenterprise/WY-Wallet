...
def test_receipt_metadata_becomes_visible_ledger_rows_and_reconciles_to_total():
    rows = materialize_receipt_adjustments(
        ...
        tax=6, service_charge=10, discount=5, fallback_category="其他",
    )
    assert [(row["item"], row["type"], row["amount"]) for row in rows[1:]] == [
        ("收据税费", EXPENSE, 6.0), ("收据服务费", EXPENSE, 10.0), ("收据折扣", REFUND, 5.0),
    ]
...
def test_adjustment_rows_inherit_unreadable_date_and_still_require_confirmation():
    rows = materialize_receipt_adjustments(
        [{"date": None, "item": "餐点", "category": "饮食", "type": EXPENSE, "amount": 100, "note": ""}], tax=6,
    )
    assert rows[-1]["date"] is None


def test_refund_adjustment_uses_schema_safe_shared_table_encoding():
    logical = db.normalize_transaction({"date": date(2026, 8, 1), "item": "收据折扣", "category": "饮食", "type": REFUND, "amount": 5, "note": ""})

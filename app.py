...
def delete_row(row_id):
    try:
        ...
        st.cache_data.clear() # ⚡️ 数据变动时强制清理缓存
...
def ai_analyze_receipt(image):
    ...
    model_name = 'gemini-1.5-flash' # 建议使用稳定版名称
    try:
...
# 先加载全局数据
df_all = load_data() 
u_years = sorted(pd.to_datetime(df_all['date']).dt.year.unique(), reverse=True) if not df_all.empty else [date.today().year]
...
with tab1:
    ...
    with col_left:
        ...
        if 'pending_data' in st.session_state:
            ...
            if not df_pending.empty:
                ...
                if 'amount' in df_pending.columns: df_pending['amount'] = df_pending['amount'].astype(float)
    ...
    with col_right:
        ...
        if not df_all.empty:
            ...
            mask = (df_history['date'].dt.year == sel_y) & (df_history['date'].dt.month == sel_m)
            df_filtered = df_history[mask]

            if not df_filtered.empty:
                st.markdown("---")
                h1, h2, h3, h4, h5, h6 = st.columns([1.2, 1.8, 1.2, 1, 0.45, 0.45])
                h1.markdown("**📅 日期**"); h2.markdown("**🏷️ 类别**"); h3.markdown("**📝 项目**"); h4.markdown("**💰 金额**"); h5.markdown("**改**"); h6.markdown("**删**")
                st.divider()
...
with tab2:
    if not df_all.empty:
        # ⚡️ 定义局部渲染片段：仅当此函数内的组件变动时，仅重新渲染此函数内容
        @st.fragment
        def render_report_section(df_input):
            st.subheader("📊 每日支出")
            ...
            if not df_plot.empty:
                ...
                fig_pie = px.pie(pie_data, values='amount', names='category', hole=0.5)

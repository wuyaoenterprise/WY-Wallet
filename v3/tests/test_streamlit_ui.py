...
@pytest.mark.parametrize(("route", "marker"), [("交易记录", "交易记录"), ("分析报表", "分析报表"), ("AI 洞察", "AI 洞察"), ("设置与备份", "设置与备份")])
...
def test_main_app_password_gate_blocks_data_until_authenticated():
    ...
    next(button for button in at.button if button.label == "进入").click().run()
    assert not at.exception
    assert bool(at.session_state["web_access_ok"]) is True


def test_receipt_page_cannot_bypass_password_gate():
    at = AppTest.from_file("v3/pages/1_📷AI收据识别.py", default_timeout=20)
    at.secrets["WEB_ACCESS_PASSWORD"] = "test-secret"
    at.run()
    assert not at.exception
    assert any("WY Wallet 私人访问" in text for text in _markdown_texts(at))
...
def test_expired_session_is_cleared_by_page_level_access_touch():
    script = r'''
import time
import streamlit as st
import wywallet.web as web
if "seeded" not in st.session_state:
    ...
    st.session_state["web_access_ok_at"] = time.time() - 1900
mode = web.touch_access_session(stop_on_expired=False)
st.write(mode)
st.write("ok" if st.session_state.get("web_access_ok") else "expired")
'''
    at = AppTest.from_string(script, default_timeout=20)
    at.secrets["WEB_ACCESS_PASSWORD"] = "test-secret"
    at.run()
    assert not at.exception
    texts = [str(element.value) for element in at.markdown]
    assert any("password-required" in text for text in texts)
    assert any("expired" in text for text in texts)

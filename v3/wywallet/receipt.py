...
def _base_key(key: tuple) -> tuple[str, str, str, float]:
    return _ensure_key(key)[:4]
...
def materialize_receipt_adjustments(transactions: Iterable[dict[str, Any]], *, tax: float = 0.0, service_charge: float = 0.0, discount: float = 0.0, fallback_category: str = "其他", receipt_id: str | None = None) -> list[dict[str, Any]]:
    rows = [dict(row) for row in transactions]
    if not rows: return rows
    first_date = next((row.get("date") for row in rows if row.get("date")), None); rid = str(receipt_id or "").strip()
    for row in rows:
        if rid: row["receipt_id"] = rid
    def add(item: str, tx_type: str, amount: float, note: str) -> None:
        if amount <= RECEIPT_TOTAL_TOLERANCE: return
        rows.append({"date": first_date, "item": item, "category": fallback_category, "type": tx_type, "amount": amount, "note": note, "receipt_id": rid})
...
def _is_duplicate(key: tuple, exact_keys: set[DuplicateKey], seen: set[tuple]) -> bool:
    ...
    if normalized in exact_keys or normalized in normalized_seen: return True
    base = normalized[:4]
...
def evaluate_receipt_candidates(edited: pd.DataFrame, existing_keys: set[tuple]) -> tuple[list[str], list[ReceiptCandidate]]:
    statuses: list[str] = []; candidates: list[ReceiptCandidate] = []; existing_duplicate_keys = _coerce_existing_duplicate_keys(existing_keys); seen: set[tuple] = set()
    for row_index, row in edited.iterrows():
        if not bool(row.get("保存")): statuses.append("未选择"); continue
        if not bool(row.get("日期已确认")): statuses.append("需确认日期"); continue
        try: normalized = normalize_transaction(row.to_dict()); key = _duplicate_key(normalized)
        except Exception as exc: statuses.append(f"无效：{exc}"); continue
        duplicate = _is_duplicate(key, existing_duplicate_keys, seen); force = bool(row.get("仍然保存重复"))
        if duplicate and not force: statuses.append("疑似重复（未保存）"); seen.add(key); continue
        status = "重复但已确认" if duplicate and force else "可保存"; statuses.append(status); candidates.append(ReceiptCandidate(int(row_index), normalized, key, force, duplicate, status)); seen.add(key)
...
def finalize_receipt_candidates(candidates: list[ReceiptCandidate], fresh_existing_keys: set[tuple]) -> tuple[list[dict], int]:
    fresh_duplicate_keys = _coerce_existing_duplicate_keys(fresh_existing_keys); seen: set[tuple] = set(); final_rows: list[dict] = []; skipped = 0
    for candidate in candidates:
        duplicate_now = _is_duplicate(candidate.key, fresh_duplicate_keys, seen)
        if duplicate_now and not candidate.force_duplicate: skipped += 1; continue
        final_rows.append(candidate.normalized); seen.add(candidate.key)
    if skipped: return [], skipped
    return final_rows, 0


def reconcile_receipt_total(candidates: list[ReceiptCandidate], receipt_total: float | None) -> dict | None:
    if receipt_total is None: return None
    expense = sum(c.normalized["amount"] for c in candidates if c.normalized["type"] == EXPENSE); refund = sum(c.normalized["amount"] for c in candidates if c.normalized["type"] == REFUND); income = sum(c.normalized["amount"] for c in candidates if c.normalized["type"] == INCOME)
    expected_total = round(expense - refund - income, 2); receipt = round(float(receipt_total), 2); difference = round(expected_total - receipt, 2)
    return {"receipt_total": receipt, "item_total": expected_total, "tax": 0.0, "service_charge": 0.0, "discount": 0.0, "expected_total": expected_total, "difference": difference, "matches": abs(difference) <= RECEIPT_TOTAL_TOLERANCE, "tolerance": RECEIPT_TOTAL_TOLERANCE}

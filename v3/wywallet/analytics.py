...
def historical_month_end_forecast(frame: pd.DataFrame, year: int, month: int, elapsed_day: int, lookback: int = 6) -> dict[str, float | int | str]:
    """Robust month-end estimate based on historical remaining-day spending.

    The median is used as the point estimate so one travel/medical month cannot
    dominate the forecast. P25/P75 are returned as a transparent uncertainty
    band. Negative net spending is preserved because refund-heavy months are
    legitimate and must not be clamped to zero.
    """

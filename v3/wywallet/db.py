...
TX_COLUMNS = ["id", "date", "item", "category", "type", "amount", "note", "receipt_id"]
...
@st.cache_resource(show_spinner=False)
def _runtime_state() -> dict[str, int]:
    return {"revision": 0}


def data_revision() -> int:
    return int(_runtime_state()["revision"])
...
def _strip_internal_markers(note: str) -> tuple[str, bool, str]:
    raw = str(note or "")
    marker_refund = False
...
def _normalize_loaded_row(row: dict[str, Any]) -> tuple[dict[str, Any] | None, list[str]]:
    ...
    note, marker_refund, receipt_id = _strip_internal_markers(str(row.get("note") or ""))
    tx_type = raw_type
    amount = raw_amount
...
def split_transaction_rows(rows: Iterable[dict[str, Any]]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    invalid_frame = pd.DataFrame(invalid, columns=["id", "date", "item", "category", "type", "amount", "note", "issues"])
...
def fetch_stable_backup_snapshot(max_attempts: int = 3) -> tuple[pd.DataFrame, pd.DataFrame, list[str], str]:
    """Return a verified full-ledger snapshot.

    The shared database does not expose a transaction-snapshot RPC to this app.
    We therefore read the full ledger twice and require identical content hashes.
    If another writer changes rows mid-read, retry rather than exporting a mixed
    time-point file. This work only runs when the user explicitly prepares backup.
    """
    previous_signature: str | None = None
    previous: tuple[pd.DataFrame, pd.DataFrame, list[str], str] | None = None
    for _ in range(max(2, int(max_attempts))):
        valid, invalid, _ = fetch_transactions_fresh(max_rows=None)
        categories = fetch_category_rows_fresh()
        signature = ledger_signature(valid) + ":" + hashlib.sha256("\n".join(sorted(categories, key=str.casefold)).encode("utf-8")).hexdigest()
        current = (valid, invalid, categories, signature)
        if previous_signature == signature:
            return current
        previous_signature = signature
        previous = current
    raise RuntimeError("备份期间账本持续发生变化，请稍后重试，避免导出混合时间点数据。")
...
def invalidate_data() -> None:
    ...
    _runtime_state()["revision"] = int(_runtime_state()["revision"]) + 1
...
def normalize_transaction(row: dict[str, Any]) -> dict[str, Any]:
    ...
    receipt_id = str(row.get("receipt_id") or "").strip()[:64]
    if not item:
    ...
    return {
        "date": tx_date.isoformat(), "item": item[:180], "category": category[:80], "type": tx_type,
        "amount": round(float(amount), 2), "note": note[:1000], "receipt_id": receipt_id,
    }


def _encode_transaction_for_db(logical: dict[str, Any]) -> dict[str, Any]:
    payload = dict(logical)
    receipt_id = str(payload.pop("receipt_id", "") or "").strip()
    user_note = str(payload.get("note") or "").strip()
    if receipt_id:
        marker = f"{RECEIPT_META_PREFIX}{receipt_id}]"
        user_note = f"{user_note} {marker}".strip()
    payload["note"] = user_note
    if payload.get("type") == REFUND:
        payload["type"] = INCOME
        payload["amount"] = abs(float(payload["amount"]))
        payload["note"] = f"{REFUND_DB_MARKER} {payload['note']}".rstrip()
    return payload
...
def insert_transactions(rows: Iterable[dict[str, Any]] | pd.DataFrame) -> int:
    ...
    get_client().table("transactions").insert([_encode_transaction_for_db(row) for row in logical_records]).execute()
    invalidate_data()
...
def update_transaction(transaction_id: int, row: dict[str, Any]) -> None:
    ...
    comparable = {k: normalized_after[k] for k in ["item", "category", "type", "amount", "note", "receipt_id"]}
    expected = {k: logical[k] for k in ["item", "category", "type", "amount", "note", "receipt_id"]}
...
def _casefold_rows(client: Client, table: str, column: str, value: str, select_columns: str) -> list[dict[str, Any]]:
    ...
    offset = 0
    while True:
        response = client.table(table).select(select_columns).ilike(column, pattern).range(offset, offset + DB_BATCH_SIZE - 1).execute()
        batch = list(response.data or [])
        for row in batch:
...
def _registered_name_variants(client: Client, name: str) -> list[str]:
    return [str(row.get("name") or "").strip() for row in _casefold_rows(client, "categories", "name", name, "name") if str(row.get("name") or "").strip()]
...
def _ids_still_at_category(client: Client, ids: list[int], category: str) -> list[int]:
    result: list[int] = []
    target = category.casefold()
    for start in range(0, len(ids), DB_BATCH_SIZE):
        chunk = ids[start:start + DB_BATCH_SIZE]
        if not chunk:
            continue
        response = client.table("transactions").select("id,category").in_("id", chunk).execute()
        for row in response.data or []:
            if str(row.get("category") or "").strip().casefold() == target:
                try:
...
def merge_category_safely(source: str, target: str) -> MergeResult:
    ...
    try:
        ...
        if _transaction_ids_for_category(client, source):
        ...
        rollback_error = None
        try:
            rollback_ids = _ids_still_at_category(client, moved_ids, target)
            for start in range(0, len(rollback_ids), DB_BATCH_SIZE):
                chunk = rollback_ids[start:start + DB_BATCH_SIZE]
                if chunk:
...
def transaction_duplicate_key(row: dict[str, Any]) -> tuple[str, str, str, float, str]:
    normalized = normalize_transaction(row)
    return (
        normalized["date"], normalized["item"].casefold(), normalized["type"],
        normalized["amount"], normalized.get("receipt_id", ""),
    )


def existing_transaction_keys(*, fresh: bool = False) -> set[tuple[str, str, str, float, str]]:
...
def transaction_duplicate_exists(row: dict[str, Any], *, fresh: bool = False) -> bool:
    try:
        return transaction_duplicate_key(row) in existing_transaction_keys(fresh=fresh)
    except ValueError:
        return False
